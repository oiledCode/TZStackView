...
public class TZStackView: UIView {
    ...
    @objc dynamic public var distribution: TZStackViewDistribution = .Fill {
        didSet {
            setNeedsUpdateConstraints()
        }
    }

    @objc dynamic public var axis: UILayoutConstraintAxis = .Horizontal {
        didSet {
            setNeedsUpdateConstraints()
        }
    }
    
    @objc dynamic public var alignment: TZStackViewAlignment = .Fill {
        didSet {
            setNeedsUpdateConstraints()
        }
    }
    ...
    private var hiddenKVOContext = UInt8()

    private var stackViewConstraintsH = NSHashTable.weakObjectsHashTable()
    private var subviewConstraintsH = NSHashTable.weakObjectsHashTable()
    ...
    required public init(coder aDecoder: NSCoder) {
        axis = aDecoder.containsValueForKey("UIStackViewAxis")
            ? UILayoutConstraintAxis(rawValue: aDecoder.decodeIntegerForKey("UIStackViewAxis")) ?? .Horizontal
            : .Horizontal

        alignment = aDecoder.containsValueForKey("UIStackViewAlignment")
            ? TZStackViewAlignment(rawValue: aDecoder.decodeIntegerForKey("UIStackViewAlignment")) ?? .Fill
            : .Fill

        distribution = aDecoder.containsValueForKey("UIStackViewDistribution")
            ? TZStackViewDistribution(rawValue: aDecoder.decodeIntegerForKey("UIStackViewDistribution")) ?? .Fill
            : .Fill

        spacing = CGFloat(aDecoder.decodeDoubleForKey("UIStackViewSpacing"))

        layoutMarginsRelativeArrangement = aDecoder.decodeBoolForKey("UIStackViewLayoutMarginsRelative")

        //baselineRelativeArrangement = aDecoder.decodeBoolForKey("UIStackViewBaselineRelative")

        let arrangedSubviews = aDecoder.decodeObjectForKey("UIStackViewArrangedSubviews") as? [UIView] ?? []
    ...
    override public func awakeFromNib() {
        ...
        let autoGeneratedConstraints = constraints.filter(constraintIsAutoGenerated)

        if #available(iOS 8.0, *) {
            NSLayoutConstraint.deactivateConstraints(autoGeneratedConstraints)
        }
        else {
            removeConstraints(autoGeneratedConstraints)
        }

        for aView in subviews where !arrangedSubviews.contains(aView) {
    ...
    private func removeHiddenListener(view: UIView) {
        if let index = registeredKvoSubviews.indexOf(view) {
            view.removeObserver(self, forKeyPath: "hidden", context: &hiddenKVOContext)
    ...
    public func addArrangedSubview(view: UIView) {
        ...
        if let autoGeneratedConstraints = Optional(view.constraints.filter(constraintIsAutoGenerated)) where autoGeneratedConstraints.count > 0 {
            if #available(iOS 8.0, *) {
                NSLayoutConstraint.deactivateConstraints(autoGeneratedConstraints)
            } else {
                view.removeConstraints(autoGeneratedConstraints)
            }
        }
    ...
    override public func updateConstraints() {
        if let autoGeneratedConstraints = Optional(constraints.filter(constraintIsAutoGenerated)) where autoGeneratedConstraints.count > 0 {
            if #available(iOS 8.0, *) {
                NSLayoutConstraint.deactivateConstraints(autoGeneratedConstraints)
            } else {
                removeConstraints(autoGeneratedConstraints)
        ...
        if let superviewAutoGeneratedConstraints = superview?.constraints.filter(compose(constraintIsAutoGenerated, constraintContains(self))) where superviewAutoGeneratedConstraints.count > 0  {
            if #available(iOS 8.0, *) {
                NSLayoutConstraint.deactivateConstraints(superviewAutoGeneratedConstraints)
            } else {
                superview!.removeConstraints(superviewAutoGeneratedConstraints)
            }
        }

        if let stackViewConstraints = stackViewConstraintsH.allObjects as? [NSLayoutConstraint] where stackViewConstraints.count > 0 {
            if #available(iOS 8.0, *) {
                NSLayoutConstraint.deactivateConstraints(stackViewConstraints)
            }
            else {
                removeConstraints(stackViewConstraints)
            }

            stackViewConstraintsH.removeAllObjects()
        }

        if let subviewConstraints = subviewConstraintsH.allObjects as? [NSLayoutConstraint] where subviewConstraints.count > 0 {
            if #available(iOS 8.0, *) {
                NSLayoutConstraint.deactivateConstraints(subviewConstraints)
            }
            else {
                for arrangedSubview in arrangedSubviews {
            ...
            subviewConstraintsH.removeAllObjects()
        }
        ...
        for arrangedSubview in arrangedSubviews {
            if alignment != .Fill {
                ...
                subviewConstraintsH.addObject(guideConstraint)
            }
            ...
            if isHidden(arrangedSubview) {
                ...
                subviewConstraintsH.addObject(hiddenConstraint)
            }
        }
        ...
        if arrangedSubviews.count > 0 {
                ...
                if distribution == .FillEqually {
                    ...
                    for (owningView, constraints) in someSubviewConstraints {
                        ...
                        for constraint in constraints {
                            subviewConstraintsH.addObject(constraint)
                        }
                    }
                }
                ...
                case .Fill, .EqualSpacing, .EqualCentering:
                    ...
                    if  totalVisible > 0 {
                        ...
                        for label in multilineLabels {
                            ...
                            if #available(iOS 8.0, *) {
                                aConstraint.active = true
                            }
                            else {
                                label.addConstraint(aConstraint)
                            }

                            subviewConstraintsH.addObject(aConstraint)
                        }
                    }
            ...
            for constraint in stackViewConstraints {
                stackViewConstraintsH.addObject(constraint)
            }
...
class TZTransformLayer: CATransformLayer {
    @objc dynamic override var opaque: Bool {
        get { return true }
        set {}
    }

    @objc dynamic override var backgroundColor: CGColor? {
        get { return nil }
        set {}
    }
}
